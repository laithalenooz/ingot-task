    ...
    <section class="container mx-auto p-6 font-mono">
        ...
        <div class="w-full mb-8 overflow-hidden rounded-lg shadow-lg">
            <div class="w-full overflow-x-auto">
                <a href="{{route('wallet.create')}}"><button class="px-4 py-2 rounded-md text-sm font-medium border focus:outline-none focus:ring transition text-gray-600 border-gray-600 hover:text-white hover:bg-gray-600 active:bg-gray-700 focus:ring-gray-300">
                    Add Balance</button></a>
                <table class="w-full">
                    <thead>
                    ...
                    @foreach($balance as $value)
                        <tr class="text-gray-700">
                            <td class="px-4 py-3 text-ms font-semibold border"><span>{{$value->income_type}}</span></td>
                            ...
                            <td class="px-4 py-3 text-sm border">
                                <form method="post" action="{{route('wallet.destroy', $value->id)}}">
                                    ...
                                    <a href="{{route('wallet.edit', $value->id)}}" class="bg-white text-gray-800 font-bold rounded border-b-2 border-green-500 hover:border-green-600 hover:bg-green-500 hover:text-white shadow-md py-2 px-6 inline-flex items-center">
